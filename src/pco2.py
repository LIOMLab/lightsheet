...
class Camera:
    ...
    def apply_settings(
        ...
        region_of_interest={'left': 1, 'right': 2560, 'top': 1, 'bottom': 2160}):
        """
    ...
    def arm(self, num_buffers=None):
        ...
        if self.very_verbose:
            print(" Camera ROI dimensions:", self.width, "(l/r) by", self.height, "(u/d)")
        ...
        for i in range(num_buffers):
            ...
            if self.very_verbose:
                print(" Buffer number ", i, " allocated, pointing to ", self.buffer_pointers[-1].contents, ", linked to event ", buffer_event.value, sep='')
    ...
    def record_to_memory(self, num_images, preframes=0, out=None, first_frame=0, poll_timeout=1e3, sleep_timeout=200, first_trigger_timeout_seconds=0):
        ...
        for which_im in range(num_images):
            ...
            while True:
                ...
                if self._dll_status.value == 0xc0008000:
                    ...
                    if self.very_verbose:
                        print(" After", self.num_polls, "polls and", self.num_sleeps, "sleeps, buffer", buffer_number, "is ready.")
                    break
                ...
                # to save CPU.
                if self.exposure_time_microseconds >= 10e3:
                    time.sleep(self.exposure_time_microseconds * 1e-6 * 2 / sleep_timeout) #Worst case (seconds)
                    self.num_sleeps += 1
            ...
            try:
                ...
                if self.very_verbose:
                    print(" Record to memory result:", hex(self._dll_status.value), hex(self._driver_status.value))
    ...
    def get_name(self):
        ...
        cam_name = C.c_char_p(b' '*40)
        dll.get_camera_name(self.camera_handle, cam_name, 40)
        self.cam_name = cam_name.value.decode('ascii')
...
try:
    dll_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), 'SC2_Cam.dll')
...
if __name__ == '__main__':
    ...
    for i in range(1):
        ...
        print(stop-start)
        images = np.zeros((1, camera.height,camera.width), dtype=np.uint16)
        for i in range(10):
            camera.record_to_memory(num_images=images.shape[0], out=images, first_trigger_timeout_seconds=5)
