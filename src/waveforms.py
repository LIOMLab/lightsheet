...
def sawtooth(
    ...
    if activated:
        # TODO: for two-way scanning, add a 'hold' between trace and retrace
        ...
        period_vector = np.concatenate(
            (pre_vector, trace_vector, retrace_vector, post_vector)
        )

        if shift != 0:
            period_vector = np.concatenate(
                (period_vector[-shift:], period_vector[:-shift])
            )
        if inverted:
        ...
        if filtered:
            ...
            tmpvec = np.concatenate(
                (period_vector[-pad:], period_vector, period_vector[:pad])
            )
...
def staircase(
    ...
    amplitude           Height of the staircase (above floor level)
                        -> Signal maximum amplitude = floor + rise
    offset              Floor level of the staircase
    direction           Either 'up' (ascending) or down (descending)

    Special case : For a staircase consisting of a single step,
    level is equal to (floor + 0.5 * rise)
    """
        ...
        if nbr_steps != 1:
            ...
            if direction == "down":
                ...
                for step in range(nbr_steps):
                    step_level = (offset + amplitude) - step * step_rise * np.ones(
                        step_run
                    )
            ...
            if shift != 0:
                output_vector = np.concatenate(
                    (output_vector[-shift:], output_vector[:-shift])
                )
            if filtered:
                # Filtering using sliding average
                ...
                # First pass (centered)
                tmpvec = np.concatenate(
                    (output_vector[:pad], output_vector, output_vector[-pad:])
                )
                cusum = np.cumsum(np.insert(tmpvec, 0, 0))
