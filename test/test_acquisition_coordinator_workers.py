...
The 5 worker methods (preview/live/single/stack mode workers + acquire_scan)
are tested by constructing the real ``AcquisitionCoordinator`` against a
mock shell with all the attributes the workers read, and exercising the
key branches: normal path, E-stop early exit, siggen error, camera timeout,
...
def test_live_mode_worker_normal_exit() -> None:
    """live_mode_worker with live_mode_started=False exits immediately."""
    acq, shell, hw = _make_acq()
    shell.live_mode_started = False
    acq.live_mode_worker()
    hw.stop_lasers.assert_called_once()
    shell.sig_live_mode_finished.emit.assert_called_once()


def test_live_mode_worker_estop_break() -> None:
    """live_mode_worker with estop_event set breaks out of the loop."""
    acq, shell, hw = _make_acq()
...
def test_single_mode_worker_estop_early_return() -> None:
    """single_mode_worker with estop_event set returns early after
    ETL standby + stop_lasers + disarm (line 251-257)."""
    acq, shell, hw = _make_acq()
    shell.estop_event.is_set.return_value = True
    acq.single_mode_worker()
    hw.stop_lasers.assert_called_once()
    shell.sig_single_mode_finished.emit.assert_called_once()


def test_single_mode_worker_normal_path() -> None:
    """single_mode_worker normal path: arm_scan, start_lasers, compute_scan,
...
def test_single_mode_worker_exception_emits_message() -> None:
    """single_mode_worker catches exceptions and emits sig_message."""
    acq, shell, hw = _make_acq()
...
def test_acquire_scan_siggen_error_aborts() -> None:
    ...
    def _fake_create_scanner():
        acq.siggen.error = 1
        acq.siggen.error_message = "DAQ error"
    acq.siggen.create_scanner = _fake_create_scanner
    acq.acquire_scan()
    shell.sig_message.emit.assert_called_once()
    assert "Scan task creation failed" in shell.sig_message.emit.call_args[0][0]


def test_acquire_scan_camera_timeout_aborts() -> None:
    ...
    acq.camera.monitor_recorder = _fake_monitor
    acq.acquire_scan()
    shell.sig_message.emit.assert_called_once()
    assert "Camera timeout" in shell.sig_message.emit.call_args[0][0]


def test_acquire_scan_normal_path_no_stitch() -> None:
    ...
    acq.camera.copy_recorder_images = Mock(return_value=np.zeros((1, 100, 100)))
...
def test_acquire_scan_normal_path_with_stitch() -> None:
    ...
    acq.camera.copy_recorder_images = Mock(return_value=np.zeros((1, 100, 100)))
    acq.camera.recorder_timeout_status = False
    shell.ui.checkBox_saveStitchBlend.isChecked.return_value = True
