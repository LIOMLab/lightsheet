...
def _laser_ao_write_0v(terminals, errors):
...
def _siggen_create_scanner(ao_terminals, do_terminals, errors):
    ...
    try:
        ...
        task_do.do_channels.add_do_chan(do_terminals,
...
def test_siggen_create_scanner_standalone():
    ...
    assert not errors, (
        'Standalone siggen create_scanner failed on Dev1:\n'
        + '\n'.join(repr(e) for _, e in errors[:5]))


def test_laser_ao_write_0v_standalone():
    ...
    assert not errors, (
        'Standalone laser AO write (0 V) failed on Dev7:\n'
        + '\n'.join(repr(e) for _, e in errors[:5]))


def test_siggen_after_laser_write_no_cascade():
