...
def test_configure_skips_stream_handler_when_stderr_is_none(
    ...
    assert not stream_handlers, "StreamHandler attached even though sys.stderr is None"
    ...
    assert file_handlers, "RotatingFileHandler not attached with no stderr"
...
def test_configure_attaches_stream_handler_when_stderr_present(
    ...
    stream_handlers = [h for h in logging.getLogger().handlers if isinstance(h, logging.StreamHandler)]
    assert len(stream_handlers) == 1, f"expected one StreamHandler, got {len(stream_handlers)}"
...
def test_configure_reads_level_and_log_dir_from_explicit_config_path(
    ...
    assert expected_log.is_file(), "RotatingFileHandler did not write to the configured log dir"
    # No ``logs/`` directory should have been created in the repo CWD.
    assert not (Path.cwd() / "logs").exists(), "configure created a CWD logs dir despite explicit Log Dir"
