...
_CONTROLLER_SRC = os.path.join(
    os.path.dirname(__file__), '..', 'gui', 'controller.py')
...
def standin():
    ...
    try:
        ...
        pass  # COM4 may be held by the running app; laser-1 path still testable
...
def test_toggle_laser1_on_daemon_thread_real_daq(standin):
    ...
    t.join(timeout=5)
    assert not errors, (
        'Daemon-thread _toggle_laser1 crashed:\n' + '\n'.join(errors))
    assert standin.lasers.error == 0, (
        'Lasers HAL error after daemon-thread toggle: '
        f'{standin.lasers.error_message}')
...
def test_write_laser1_power_real_daq_repeated(standin):
    ...
    for _ in range(15):
        try:
            ...
            errors.append(repr(e))
    assert not errors, (
        '_write_laser1_power crashed on real DAQ:\n' + '\n'.join(errors))
    assert standin.lasers.error == 0, (
        'Lasers HAL error after repeated 0 V writes: '
        f'{standin.lasers.error_message}')
...
def test_start_lasers_real_daq_then_siggen_create(standin):
    ...
    if have_siggen:
        ...
        assert siggen.error == 0, (
            f'siggen.create_scanner failed after start_lasers: '
            f'{siggen.error_message}')
        try:
