...
def test_goto_buttons_exist(qtbot: QtBot, controller: Controller_MainWindow) -> None:
    ...
    assert isinstance(
        ctrl.stack_panel.ui.pushButton_acqGoToFirstPlane, QPushButton
    )
...
def test_goto_start_calls_move_absolute_with_um(
    ...
    with patch.object(
        ctrl.motors.horizontal, "move_absolute_position"
    ) as mock_move:
...
def test_goto_end_out_of_range_beeps_and_does_not_move(
    ...
    ctrl.stack_ending_plane = (
        ctrl.motors.horizontal.get_limit_high("μm") + 1000.0
    )
