...
- The master enable toggle hides only the fields container.
- ``build_focus_config`` and ``build_focus_curve`` return ``None`` after
  the legacy per-block focus controls were removed.
- ``build_autofocus_config`` returns ``None`` when disabled and a frozen
  ``AutofocusConfig`` when enabled.
- Adaptive-autofocus spinboxes clamp out-of-range values.
"""
...
def test_set_autofocus_running_toggles_progress_and_disables_controls(
    ...
    assert not ui.checkBox_focusEnable.isEnabled()
    ...
    assert ui.checkBox_focusEnable.isEnabled()
