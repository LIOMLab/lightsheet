...
def test_estop_acquires_no_laser_lock(
    ...
    ctrl.lasers[0]._lock = _NoLockAcquire()  # ty: ignore[invalid-assignment]
    ctrl.lasers[1]._lock = _NoLockAcquire()  # ty: ignore[invalid-assignment]
...
def test_refresh_laser2_readback_lock_skip_is_noop(
    ...
    ctrl.timer_imageview.stop()
    ctrl.timer_laser2_status.stop()
    ctrl._hw.lasers[1]._lock = threading.Lock()  # ty: ignore[invalid-assignment]
