...
def test_preview_worker_no_data_breaks_and_cleans(qtbot: QtBot) -> None:
    ...
    worker.camera.disarm = disarm_spy
...
def test_live_mode_worker_acquire_scan_does_not_skip_cleanup(qtbot: QtBot) -> None:
    ...
    worker.acquire_scan = _acquire_then_stop  # ty: ignore[invalid-assignment]
...
def test_acquire_scan_no_data_aborts_and_cleans_up(qtbot: QtBot) -> None:
    ...
    worker.camera.delete_recorder = delete_recorder_spy
    delete_scanner_spy = Mock(wraps=worker.siggen.delete_scanner)
    worker.siggen.delete_scanner = delete_scanner_spy
    disarm_spy = Mock(wraps=worker.camera.disarm)
    worker.camera.disarm = disarm_spy
...
def _make_d04_shell(
    ...
    hw = HardwareManager(bundle, shell)  # ty: ignore[invalid-argument-type]
...
def test_live_worker_exception_in_cleanup_emits_message(qtbot: QtBot) -> None:
    ...
    shell.live_mode_started = False
    worker.camera.disarm = Mock(side_effect=RuntimeError("disarm fault"))
    finished_emits: list[None] = []
    worker.finished.connect(lambda: finished_emits.append(None))
    worker.run()
    shell.sig_message.emit.assert_called_once()
...
def test_live_worker_exception_in_etl_and_laser_cleanup(qtbot: QtBot) -> None:
    ...
    worker, shell, _hw = _make_live_worker(qtbot)
    shell.live_mode_started = False
    worker.siggen.update_etls = Mock(side_effect=RuntimeError("etl fault"))
    worker._hw.stop_lasers = Mock(side_effect=RuntimeError("stop_lasers fault"))
    finished_emits: list[None] = []
    worker.finished.connect(lambda: finished_emits.append(None))
    worker.run()
    shell.sig_message.emit.assert_called_once()
...
def test_preview_worker_runs_one_frame_then_exits(qtbot: QtBot) -> None:
    ...
    worker.camera.copy_recorder_images = _copy_one_frame  # ty: ignore[invalid-assignment]
    finished_emits: list[None] = []
    worker.finished.connect(lambda: finished_emits.append(None))
    worker.run()
...
def test_live_worker_exception_in_start_lasers_emits_message(qtbot: QtBot) -> None:
    ...
    worker, shell, _hw = _make_live_worker(qtbot)
    shell.live_mode_started = False
    worker._hw.start_lasers = Mock(side_effect=RuntimeError("laser fault"))
    finished_emits: list[None] = []
    worker.finished.connect(lambda: finished_emits.append(None))
    worker.run()
    shell.sig_message.emit.assert_called_once()
...
def test_single_worker_multi_channel_second_acquire_fails(qtbot: QtBot) -> None:
    ...
    worker, shell, hw = _make_single_worker_multi(qtbot, multi_channel=True)
    shell.estop_event.is_set.return_value = False
    worker.acquire_scan = Mock(side_effect=[True, False])
    worker.camera.recorder_timeout_status = False
    worker.siggen.error = 0
    finished_emits: list[None] = []
    worker.finished.connect(lambda: finished_emits.append(None))
...
def test_single_worker_multi_channel_none_frames_skips_enqueue(qtbot: QtBot) -> None:
    ...
    worker.acquire_scan = acquire_mock
    finished_emits: list[None] = []
    worker.finished.connect(lambda: finished_emits.append(None))
    worker.run()
