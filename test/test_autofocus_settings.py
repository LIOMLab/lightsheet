...
def test_autofocus_settings_rejects_cadence_out_of_range() -> None:
    with pytest.raises(ValidationError):
        AutofocusSettings(**{**_autofocus_valid(), "Cadence": 0})
    with pytest.raises(ValidationError):
        AutofocusSettings(**{**_autofocus_valid(), "Cadence": 1001})


def test_autofocus_settings_rejects_residual_gain_out_of_range() -> None:
    with pytest.raises(ValidationError):
        AutofocusSettings(**{**_autofocus_valid(), "Residual Gain Mm": 1.5})
    with pytest.raises(ValidationError):
        AutofocusSettings(**{**_autofocus_valid(), "Residual Gain Mm": -0.1})


def test_autofocus_settings_rejects_max_residual_out_of_range() -> None:
    with pytest.raises(ValidationError):
        AutofocusSettings(**{**_autofocus_valid(), "Max Residual Mm": 5.5})
    with pytest.raises(ValidationError):
        AutofocusSettings(**{**_autofocus_valid(), "Max Residual Mm": -0.1})


def test_autofocus_settings_rejects_smoothing_out_of_range() -> None:
    with pytest.raises(ValidationError):
        AutofocusSettings(**{**_autofocus_valid(), "Smoothing": -0.1})
    with pytest.raises(ValidationError):
        AutofocusSettings(**{**_autofocus_valid(), "Smoothing": 1.5})


def test_autofocus_settings_rejects_unknown_key() -> None:
    with pytest.raises(ValidationError) as exc_info:
        AutofocusSettings(**{**_autofocus_valid(), "Unknown Key": 1})
...
def test_autofocus_overlay_tolerates_extra_key() -> None:
    s = AutofocusSettingsOverlay(**{**_autofocus_valid(), "Unknown Key": 1})
    assert s.cadence == 1
    assert s.smoothing == 0.5
