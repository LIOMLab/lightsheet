...
class StackWorker(QObject, _AcquireScanMixin, _StackAdaptiveMixin):
        ...
        *,
        ...
        if snapshot is not None:
            ...
            try:
                ...
                # A broken model silently degrading to the legacy path
                # hides the integration faults the model exists to
                # surface — log before falling back.
            ...
            if not isinstance(base, MicroscopeSnapshot):
                ...
                # Prefer the camera's configured line time over a
                # hardcoded 1 s default far outside the widget's µs range.
                line_time = getattr(
                    getattr(shell, "camera", None), "lightsheet_line_time", None
                )
                if (
    ...
    def run(self) -> None:
        ...
        # The plane loop resets the watchdog once per plane; store the
        # handle on self so the loop body reaches it without widening
        # the (n_planes, remaining_planes) signature.
        self._watchdog = watchdog
        try:
            setup = self._run_setup(watchdog)
            if setup is not None:
                n_planes, remaining_planes = setup
                self._run_plane_loop(n_planes, remaining_planes)
        except Exception as e:
    ...
    def _run_setup(
        ...
        Returns ``(n_planes, remaining_planes)`` for the plane loop, or
        ``None`` when the run aborts before the first plane (zero plane
        count or an unreadable autofocus seed position) — the abort's
        message and beep are emitted here and ``run()`` still reaches
        the teardown through ``finally``.
        """
        ...
        return n_planes, remaining_planes

    def _run_plane_loop(self, n_planes: int, remaining_planes: int) -> None:
        """Run the per-plane acquisition loop: loop-top cooperative
        interruption / Stop / E-stop / pause polls, the motor move,
        ``acquire_scan``, the save enqueue, and the progress emit.
        ``self._run_completed`` is set only when the loop exhausts
        without a break — every break path leaves it False so the
        teardown finalizes the manifest as interrupted or paused.
        """
        # Local alias so the per-plane reset below reads as it did when
        # the loop lived in run(); run() stores the handle on self.
    ...
    def _run_teardown(self, watchdog: LaserWatchdog | None) -> None:
        """Run the teardown contract: manifest-lifecycle ``stop_saving``,
        ETL standby, lasers off, watchdog disarm, camera disarm, the
        cleanup-error summary, ``finished.emit()``, and the pause-event
        clear LAST. Each cleanup step is individually guarded into
        ``_cleanup_errors``; ``finished`` and the pause clear run even
        when a step throws.
        """
