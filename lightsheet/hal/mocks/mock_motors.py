"""Standalone mock Motors HAL for demo mode.

``MockMotors`` (container) + ``MockMotor`` (per-axis) implement ``IMotors`` /
``IMotor`` with no serial I/O, tracking per-axis position in software.

Travel-limit enforcement is preserved: ``move_absolute_position`` and
``move_relative_position`` raise ``ValueError`` on over-travel BEFORE any
state change, so safety checks do not atrophy under demo mode.
"""
...
class MockMotor(IMotor):
    """Mock per-axis Zaber stage for demo mode -- implements
    IMotor with no serial I/O."""
    ...
    def move_absolute_position(self, absolute_position: float, units: str) -> None:
        ...
        BEFORE any state change (safety: must not atrophy under demo)."""
    ...
    def move_relative_position(self, relative_position: float, units: str) -> None:
        """Move by a relative delta. The resulting position is validated against
        travel limits BEFORE the move. Raises ``ValueError`` on over-travel."""
    ...
    def position_to_microsteps(self, position: float, units: str = "mm") -> int:
        ...
        matching the effective ``ZaberMotor._motorIO`` behavior."""
        if units == "m":
            factor = 1
        elif units == "cm":
            factor = pow(10, -2)
...
class MockMotors(IMotors):
    ...
    def __init__(self) -> None:
        ...
        # Synthetic defaults mirroring real Zaber T-LS stage families.
