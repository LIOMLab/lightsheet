...
convention as the legacy ``scripts/daqmx.py`` / ``scripts/h5test.py`` scripts
...
----------------------------------------
    uv run python scripts/laser1_calibration_sweep.py
    uv run python scripts/laser1_calibration_sweep.py --step 0.1 --samples 5
    uv run python scripts/laser1_calibration_sweep.py --warmup-secs 300
    uv run python scripts/laser1_calibration_sweep.py --no-descending
    uv run python scripts/laser1_calibration_sweep.py --output laser1_calibration.csv
    uv run python scripts/laser1_calibration_sweep.py --skip-zero
...
def _write_csv(output: str, pairs: list[tuple[float, float, str]]) -> None:
    ...
    with Path(output).open("w", newline="") as f:
        f.write(
            ...
            f"# Generated by scripts/laser1_calibration_sweep.py (v4)\n"
        )
        writer = csv.writer(f)
